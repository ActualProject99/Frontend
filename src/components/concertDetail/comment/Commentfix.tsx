...
const Commentfix = ({comment} : IComments) => {
  ...
  const onEdit = (data: any) => {
      ...
      if(askConfirm) {
        /* const {id, postId} = comment; */
      editCommentFn({id: comment?.id, body: { postId: comment?.postId, comment:data.editcomment}});
      setIsEdit((cur)=>!cur)
      }
  };
        ...
        {isedit ? (
          <>
           <form onSubmit={handleSubmit(onEdit)} className="flex ">
            <textarea className="border 1px w-5/6 h-28 placeholder: pb-12 pl-4 rounded-lg rounded-r-none rounded-br-none resize-none"
            ...
            <button 
              className="border 1px w-[10.5rem] h-[7.1rem] hover:bg-secondary-main bg-secondary-300"
            >
              등록
            </button>
            <button 
            ...
            {errors.editcomment && errors.editcomment?.type === "minLength" && <span className='font-bold text-sm text-red-600'>{`${errors.editcomment.message}`}</span>}
            </>
        ) : (
          <>
          {comment?.comment}
